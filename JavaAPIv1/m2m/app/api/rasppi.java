...
public class rasppi {
	...
	protected String SetGPIOfunction(int pin, boolean fun)
	{
		...
		//removes persistence if function is set to input
		if(fun == GPIO_FUNCTION_IN)
		{
			(this.getPinDeep(pin)).removePersistence();
		}
	...
	protected String SetGPIOvalue(int pin, boolean val)
	{
		...
		//persistent pins must have their values changed before update()
		//otherwise they will be reset to old values
		pipin currentPin = this.getPinDeep(pin);
		if(currentPin.isPersistent())
		{
			currentPin.setState(val);
		}
	...
	protected boolean isPinPersitent(int index, boolean deep)
	{
		pipin currentPin = null;
		if(deep)
		{
		...
		return currentPin.isPersistent();
	}
	...
	//setter methods for persistence
	protected boolean addPinPersistence(int index, boolean deep)
	{
		pipin currentPin = null;
		if(deep)
		{
		...
		return currentPin.addPersistence();
	}
	
	protected boolean removePinPersistence(int index, boolean deep)
	{
		pipin currentPin = null;
		if(deep)
		{
		...
		return currentPin.removePersistence();
	}
	...
	private void addPin(int pin_number, boolean pin_function, String pin_name, boolean pin_state, boolean pin_persistence, String pin_high_label, String pin_low_label)
	{
		pipin newPin = new pipin(pin_function, pin_number, pin_name, pin_state, pin_persistence, pin_high_label, pin_low_label);
		GPIO.add(newPin);
	}
	...
	protected void update()
	{
		...
		for (int i = 0; i < pinTotal; i++) 
		{
			...
			if(current_pin.isPersistent() && current_pin.getFunction() == GPIO_FUNCTION_OUT)
			{
				int current_pin_number = current_pin.getNumber();
				boolean current_pin_state = current_pin.getState();
				this.SetGPIOfunction(current_pin_number, GPIO_FUNCTION_OUT);
				this.SetGPIOvalue(current_pin_number, current_pin_state);
			}
		}
	...
	protected ArrayList<PinInfoObj> getPinInfoArray()
	{
		...
		for(int i = 0; i < max; i++)
		{
				...
				PinInfoObj infoPin =  new PinInfoObj(nextPin.getNumber(), nextPin.getName(), nextPin.getFunction(), nextPin.getState(), nextPin.isPersistent(), nextPin.getHighLabel(), nextPin.getLowLabel());
