...
//            self.reloadInputViews();
    ...
    func listDevices()
    {
        ...
        let listtask = URLSession.shared.dataTask(with: request) {
            urlData, response, error in
            ...
            DispatchQueue.main.async {
                ...
                if(urlData != nil) {
                    ...
                    do {
                        guard let jsonData = try JSONSerialization.jsonObject(with: urlData!, options: []) as? [String: AnyObject] else {
                        ...
                        //REMOVE SSH ENTRIES
                        var temparray = self.devices;
                        var sshindexarr = [Int]();
                        var i = 0;
                        for item in temparray! {
                            let alias: String? = item.value(forKey: "devicealias") as? String
                        ...
                        for inx in sshindexarr {
                            temparray!.remove(at: inx);
                        }*/
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        // BUG: devices is nil at start of app 
        return 2;//devices!.count;
    }
...
//        body.setValue("true", forKey: "wait");
        ...
        let weblogtask = URLSession.shared.dataTask(with: request) {
            urlData, response, error in
            ...
            DispatchQueue.main.async {
                ...
                if(urlData != nil) {
                    ...
                    if (status == "false"){
                        ...
                        let proxy = jsonData["connection"]?["proxy"] as! String;
    ...
    func devFetchTest()
    {
        ...
        let task = tbc.session.dataTask(with: request) {
            ...
            DispatchQueue.main.async {
                ...
                //jsonData is where the data for the response is kept
                let jsonData = try! JSONSerialization.jsonObject(with: urlData!, options: []) as? [String: AnyObject];
                
                    print("successful fetch from Pi");
                let pinarray = jsonData?["GPIO"] as! [String: AnyObject];
                    print(pinarray["0"] as Any);
                    tbc.getPins();
