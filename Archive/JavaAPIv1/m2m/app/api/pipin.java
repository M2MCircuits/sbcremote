...
public class pipin {
	...
	private boolean pin_persistence; //whether the pin resumes its function and state after powerout
	...
	pipin( boolean pfun, int pinum, String pinam, boolean pist, boolean pipers, String pihi, String pilo)
	{
		pin_function = pfun;
		pin_number = pinum;
		pin_name = pinam;
		pin_state = pist;
		pin_persistence = pipers;
		pin_high_label = pihi;
		pin_low_label = pilo;
	}
	...
	{
		//input pins cannot be persistent
		if(gf == GPIO_FUNCTION_IN)
		{
	...
	protected boolean addPersistence()
	{
		//only output pins can be persistent
		if (this.getFunction() == GPIO_FUNCTION_OUT) 
		{
	...
	protected boolean removePersistence()
	{
		pin_persistence = false;
		return pin_persistence;
	}
	...
	{
		return "{" + "\"number\":" + "\"" + this.getNumber() + "\"" + ","  + "\"function\":" + "\"" + this.getFunction() + "\"" + ","  + "\"name\":" + "\"" + this.getName() + "\"" + ","  + "\"state\":" + "\"" + this.getState() + "\"" + ","  + "\"persistent\":" + "\"" + this.isPersistent() + "\"" + ","  + "\"high\":" + "\"" + this.getHighLabel() + "\"" + ","  + "\"low\":" + "\"" + this.getLowLabel() + "\"" + "}";
	}
